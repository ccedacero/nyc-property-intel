...
Queries the local nypd_crime_complaints table (bulk-loaded from NYC Open Data
dataset 5uac-w243) using a geospatial bounding-box search centered on the
property's coordinates. Falls back to the Socrata API if the local table is
unavailable.

Radius-based querying is more accurate than address string matching for crime
data because incidents are geocoded to the crime scene, not the complainant's
address.
...
_SOCRATA_DATASET = "5uac-w243"

# ── SQL ───────────────────────────────────────────────────────────────────────

# Bounding-box radius approximation. At NYC's latitude (~40.7°):
#   1° lat  ≈ 111,000 m  →  1 m ≈ 0.000009009°
#   1° lon  ≈  84,700 m  →  1 m ≈ 0.000011806°
# This is accurate to within ~5% for the radius sizes we support (50–800 m).
_SQL_LOCAL = """\
SELECT
...
FROM nypd_crime_complaints
WHERE latitude  IS NOT NULL
  AND longitude IS NOT NULL
  AND latitude  BETWEEN $1 AND $2
  AND longitude BETWEEN $3 AND $4
  AND ($5::text IS NULL OR upper(law_cat_cd) = upper($5))
  AND ($6::text IS NULL OR upper(ofns_desc) LIKE '%' || upper($6) || '%')
  AND ($7::text IS NULL OR cmplnt_fr_dt >= $7)
ORDER BY cmplnt_fr_dt DESC
LIMIT $8;
"""

_LAT_PER_METER  = 1.0 / 111_000.0
_LON_PER_METER  = 1.0 /  84_700.0
...
def _bbox(lat: float, lon: float, radius_m: int) -> tuple[float, float, float, float]:
    ...
    dlat = radius_m * _LAT_PER_METER
    dlon = radius_m * _LON_PER_METER
...
async def _resolve_lat_lon(
    ...
    """Return (lat, lon, label) for a BBL or address."""
    # ── BBL path: PLUTO centroid ──────────────────────────────────────
    if bbl:
        row = await fetch_one(
...
async def get_nypd_crime(
    ...
    lat_min, lat_max, lon_min, lon_max = _bbox(lat, lon, radius_meters)

    # ── Local DB bounding-box query ───────────────────────────────────
    try:
        ...
        logger.info("nypd_crime_complaints not found — falling back to Socrata")

    # ── Socrata fallback ──────────────────────────────────────────────
    parts: list[str] = [f"within_circle(lat_lon, {lat}, {lon}, {radius_meters})"]
    if law_category:
        parts.append(f"upper(law_cat_cd) = '{_soql_escape(law_category.upper())}'")
    if offense:
        parts.append(f"upper(ofns_desc) like '%{_soql_escape(offense.upper())}%'")
    if since_year:
    ...
    return {
        ...
        "summary": _summarize(incidents_raw),
